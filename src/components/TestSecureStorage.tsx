...
const TestSecureStorage: React.FC = () => {
  ...
  const resetAll = () => {
    if (
      window.confirm(
        "⚠️ ATTENTION: Cela va supprimer TOUTES les données du localStorage. Continuer?"
      )
    ) {
      const output = captureConsole(() => {
        resetStorage();
    ...
    <div className="max-w-4xl mx-auto p-6 bg-gray-900 text-white rounded-lg">
      ...
      <div className="mt-6 text-sm text-gray-400">
        ...
        <ul className="list-disc list-inside space-y-1">
          <li>
            <strong>🚀 Tests</strong> : Suite complète de tests du système
          </li>
          <li>
            <strong>🔍 Diagnostic</strong> : Analyse l'état du localStorage
          </li>
          <li>
            <strong>🧹 Nettoyer</strong> : Supprime les données corrompues
          </li>
          <li>
            <strong>🗑️ Tests</strong> : Nettoie les données de test
          </li>
          <li>
            <strong>🚨 Reset</strong> : Supprime TOUTES les données (destructif)
          </li>
        </ul>
      </div>
    </div>
